...
from .reactions_mixin import TelegramReactionsMixin
from .payloads import (
    _clip_text,
    _compact_prompt_payload,
    _decision_to_payload,
    _first_reply_excerpt,
    _format_origin_ref,
    _goal_summary,
    _is_unavailable_media_description,
    _memory_pair_view,
    _minimal_prompt_payload,
    _strip_telegram_prefix,
    _tool_event_payload,
)
...
class NikolaBot(TelegramAttachmentMixin, TelegramStickerMixin, TelegramReactionsMixin):
    def __init__(
        self,
        *,
    ...
    def process_update(self, update: dict[str, Any]) -> bool:
        ...
        if not decision.should_reply and not decision.stickers and not decision.reactions:
        ...
        self._apply_reactions(
            chat_state,
            reply_to_message_id if isinstance(reply_to_message_id, int) else current_message_id or None,
            decision.reactions,
        )
        self._schedule_from_minutes(chat_state.chat_id, decision.next_check_minutes)
        return True
    ...
    def decide_incoming(
        ...
        if (
            ...
            and not decision_reply_texts(decision)
            and not decision.stickers
        ):
            # Pure reaction-only turns are valid: keep should_reply truthy
            # so _handle_incoming_message routes through the apply path.
            if not decision.reactions:
                decision.should_reply = False
